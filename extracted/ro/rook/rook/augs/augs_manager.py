...
class AugsManager(object):
    ...
    def add_aug(self, configuration):
        try:
            ...
            if len(self._augs) > self._augs_limit:
                message = "Breakpoint limit exceeded: %d" % (len(self._augs))
                self._output.send_rule_status(aug_id, "Error", Error(message=message))
                return
        except Exception as exc:
            logger.exception("Failed to get aug id", exc)
