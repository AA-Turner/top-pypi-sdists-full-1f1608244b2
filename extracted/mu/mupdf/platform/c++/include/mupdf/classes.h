...
struct FzBitmap
{
	...
	*/
	FZ_FUNCTION void fz_bitmap_details(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *w
...
struct FzBuffer
{
	...
	*/
	FZ_FUNCTION size_t fz_buffer_extract(
			#ifdef SWIG
	...
	*/
	FZ_FUNCTION size_t fz_buffer_storage(
			#ifdef SWIG
				unsigned char **OUTPUT
			#else
				unsigned char **datap
	...
	*/
	FZ_FUNCTION FzDisplayList fz_new_display_list_from_svg(const char *base_uri, const FzArchive& zip, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *w
...
struct FzColorspace
{
	...
	*/
	...
	/**
		...
		given colorspace.
	*/
	FZ_FUNCTION void fz_clamp_color(const float *in, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *out
	...
	*/
	FZ_FUNCTION void fz_convert_color(const float *sv, const FzColorspace& ds, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *dv
			#endif
			, const FzColorspace& is, const FzColorParams& params) const;
	...
	*/
	FZ_FUNCTION void fz_convert_separation_colors(const float *src_color, const FzSeparations& dst_seps, const FzColorspace& dst_cs, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *dst_color
			#endif
			, const FzColorParams& color_params) const;
...
struct FzCompressedBuffer
{
	...
	*/
	FZ_FUNCTION FzStream fz_open_image_decomp_stream_from_buffer(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *l2factor
...
struct FzDevice
{
	...
	*/
	...
	FZ_FUNCTION PdfProcessor pdf_new_run_processor(const FzMatrix& ctm, const char *usage, const PdfGstate& gstate, const FzDefaultColorspaces& default_cs, FzCookie& cookie) const;
...
struct FzDisplayList
{
	...
	*/
	...
	/**
		Parse an SVG document into a display-list.
	*/
	FZ_FUNCTION FzDisplayList(const FzBuffer& buf, const char *base_uri, const FzArchive& zip, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *w
	...
	/**
		Parse an SVG document into a display-list.
	*/
	FZ_FUNCTION FzDisplayList(const FzXml& xmldoc, const FzXml& xml, const char *base_uri, const FzArchive& zip, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *w
	...
	*/
	 ...
	 */
	FZ_FUNCTION int fz_search_display_list(const char *needle, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *hit_mark
			#endif
			, FzQuad& hit_bbox, int hit_max) const;
...
struct FzDocument
{
	...
	*/
	...
	/** Return FzDocument for pdfdocument.m_internal.super. */
	...
	*/
	FZ_FUNCTION FzLocation fz_resolve_link(const char *uri, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *xp
	...
	*/
	 ...
	 */
	FZ_FUNCTION int fz_search_chapter_page_number(int chapter, int page, const char *needle, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *hit_mark
	 ...
	 */
	FZ_FUNCTION int fz_search_page_number(int number, const char *needle, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *hit_mark
...
struct FzDocumentWriter
{
	...
	*/
	...
	FZ_FUNCTION void fz_write_story(const FzStoryS& story, ::fz_write_story_rectfn rectfn, void *rectfn_ref, ::fz_write_story_positionfn positionfn, void *positionfn_ref, ::fz_write_story_pagefn pagefn, void *pagefn_ref) const;
...
struct FzImage
{
	...
	*/
	FZ_FUNCTION FzImage(int w, int h, int bpc, const FzColorspace& colorspace, int xres, int yres, int interpolate, int imagemask, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *decode
	...
	*/
	FZ_FUNCTION FzPixmap fz_get_pixmap_from_image(FzIrect& subarea, FzMatrix& ctm, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *w
	...
	*/
	...
	/**
		...
		if not encoded).
	*/
	FZ_FUNCTION void fz_image_resolution(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *xres
...
struct FzOutput
{
	...
	*/
	...
	/**
		...
		id: ID parameter to keep generated IDs unique across SVG files.
	*/
	FZ_FUNCTION FzDevice fz_new_svg_device_with_id(float page_width, float page_height, int text_format, int reuse_images, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *id
...
struct FzPage
{
	...
	*/
	FZ_FUNCTION FzTransition fz_page_presentation(FzTransition& transition, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *duration
...
struct FzPixmap
{
	...
	*/
	FZ_FUNCTION void fz_fill_pixmap_with_color(const FzColorspace& colorspace, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *color
...
struct FzRect
{
	...
	*/
	...
	/** Default constructor initialises to (0, 0, 0, 0. */
	FZ_FUNCTION FzRect();
...
struct FzSeparations
{
	...
	*/
	FZ_FUNCTION void fz_separation_equivalent(int idx, const FzColorspace& dst_cs, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *dst_color
			#endif
			, const FzColorspace& prf, const FzColorParams& color_params) const;
...
struct FzShadeColorCache
{
	/** Constructor that sets m_internal to NULL; can then be passed to FzShade::fz_paint_shade(). */
...
struct FzStextPage
{
	...
	*/
	...
	/**
		...
		without notice.
	*/
	FZ_FUNCTION int fz_search_stext_page(const char *needle, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *hit_mark
...
/** Wrapper class for struct `fz_story_element_position`. Not copyable or assignable. */
struct FzStoryElementPosition
{
	/** Constructor using raw copy of pre-existing `::fz_story_element_position`. */
	FZ_FUNCTION FzStoryElementPosition(::fz_story_element_position* internal);
	...
	/** Pointer to wrapped data. */
	::fz_story_element_position* m_internal;

	private:

	/** This class is not copyable or assignable. */
	FzStoryElementPosition(const FzStoryElementPosition& rhs);
	FzStoryElementPosition& operator=(const FzStoryElementPosition& rhs);
};

/** Wrapper class for struct `fz_story_s`. Not copyable or assignable. */
struct FzStoryS
{
	/** == Constructors. */

	/** Constructor using `fz_new_story()`. */
	FZ_FUNCTION FzStoryS(const FzBuffer& buf, const char *user_css, float em, const FzArchive& archive);
	...
	/** Constructor using raw copy of pre-existing `::fz_story_s`. */
	FZ_FUNCTION FzStoryS(::fz_story_s* internal);
	...
	/** Pointer to wrapped data. */
	::fz_story_s* m_internal;

	private:

	/** This class is not copyable or assignable. */
	FzStoryS(const FzStoryS& rhs);
	FzStoryS& operator=(const FzStoryS& rhs);
};
...
struct FzStream
{
	...
	*/
	FZ_FUNCTION FzPixmap fz_decomp_image_from_stream(const FzCompressedImage& image, FzIrect& subarea, int indexed, int l2factor, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *l2extra
	...
	*/
	...
	/**
		...
		with optional log2 subsampling.
	*/
	FZ_FUNCTION FzStream fz_open_image_decomp_stream(const FzCompressionParams& arg_1, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *l2factor
	...
	*/
	FZ_FUNCTION FzBuffer fz_read_best(size_t initial, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *truncated
			#endif
			, size_t worst_case) const;
...
struct FzXml
{
	...
	*/
	FZ_FUNCTION const char *fz_dom_get_attribute(int i, 
			#ifdef SWIG
				const char **OUTPUT
			#else
				const char **att
	...
	*/
	...
	/**
		Parse an SVG document into a display-list.
	*/
	FZ_FUNCTION FzDisplayList fz_new_display_list_from_svg_xml(const FzXml& xml, const char *base_uri, const FzArchive& zip, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *w
...
struct PdfAnnot
{
	 ...
	 */
	FZ_FUNCTION void pdf_annot_MK_BC(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *n
	 ...
	 */
	FZ_FUNCTION void pdf_annot_MK_BG(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *n
	 ...
	 */
	FZ_FUNCTION void pdf_annot_color(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *n
	 ...
	 */
	FZ_FUNCTION void pdf_annot_default_appearance(
			#ifdef SWIG
				const char **OUTPUT
			#else
				const char **font
	 ...
	 */
	FZ_FUNCTION void pdf_annot_interior_color(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *n
	 ...
	 */
	FZ_FUNCTION void pdf_annot_line_ending_styles(
			#ifdef SWIG
	 ...
	 */
	FZ_FUNCTION int pdf_edit_text_field_value(const char *value, const char *change, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *selStart
...
struct PdfCmap
{
	 ...
	 */
	FZ_FUNCTION int pdf_decode_cmap(unsigned char *s, unsigned char *e, 
			#ifdef SWIG
				unsigned int *OUTPUT
			#else
				unsigned int *cpt
	 ...
	 */
	FZ_FUNCTION int pdf_lookup_cmap_full(unsigned int cpt, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *out
	 ...
	 */
	FZ_FUNCTION void pdf_map_one_to_many(unsigned int one, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *many
...
struct PdfCrypt
{
	...
	FZ_FUNCTION char *pdf_crypt_method() const;
...
struct PdfDocument
{
	 ...
	 */
	FZ_FUNCTION int pdf_field_event_validate(const PdfObj& field, const char *value, 
			#ifdef SWIG
				char **OUTPUT
			#else
				char **newvalue
	...
	FZ_FUNCTION void pdf_filter_annot_contents(const PdfAnnot& annot, PdfFilterOptions& filter) const;

	/** Class-aware wrapper for `::pdf_filter_page_contents()`.  */
	FZ_FUNCTION void pdf_filter_page_contents(const PdfPage& page, PdfFilterOptions& filter) const;
	 ...
	 */
	FZ_FUNCTION void pdf_load_to_unicode(const PdfFontDesc& font, 
			#ifdef SWIG
				const char **OUTPUT
			#else
				const char **strings
			#endif
			, char *collection, const PdfObj& cmapstm) const;
	 ...
	 */
	FZ_FUNCTION int pdf_lookup_anchor(const char *name, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *xp
	...
	FZ_FUNCTION PdfProcessor pdf_new_filter_processor(const PdfProcessor& chain, const PdfObj& old_res, const PdfObj& new_res, int struct_parents, const FzMatrix& transform, PdfFilterOptions& filter) const;
	 ...
	 */
	FZ_FUNCTION PdfObj pdf_parse_ind_obj(const FzStream& f, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *num
	 ...
	 */
	FZ_FUNCTION PdfObj pdf_parse_journal_obj(const FzStream& stm, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *onum
	 ...
	 */
	FZ_FUNCTION int pdf_repair_obj(const PdfLexbuf& buf, 
			#ifdef SWIG
				int64_t *OUTPUT
			#else
				int64_t *stmofsp
	 ...
	 */
	FZ_FUNCTION int pdf_resolve_link(const char *uri, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *xp
	 ...
	 */
	FZ_FUNCTION size_t pdf_signature_contents(const PdfObj& signature, 
			#ifdef SWIG
				char **OUTPUT
			#else
				char **contents
	 ...
	 */
	FZ_FUNCTION int pdf_undoredo_state(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *steps
...
struct PdfFilterOptions2 : PdfFilterOptions
{
	...
	FZ_FUNCTION virtual void end_page(struct ::fz_context *arg_0, void *arg_2);
};
...
struct PdfFunction
{
	 ...
	 */
	FZ_FUNCTION void pdf_eval_function(const float *in, int inlen, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *out
...
struct PdfJs
{
	 ...
	 */
	FZ_FUNCTION int pdf_js_event_result_validate(
			#ifdef SWIG
				char **OUTPUT
			#else
				char **newvalue
	 ...
	 */
	FZ_FUNCTION void pdf_js_execute(const char *name, const char *code, 
			#ifdef SWIG
				char **OUTPUT
			#else
				char **result
...
struct PdfObj
{
	 ...
	 */
	FZ_FUNCTION const char *pdf_array_get_string(int index, 
			#ifdef SWIG
				size_t *OUTPUT
			#else
				size_t *sizep
	 ...
	 */
	FZ_FUNCTION const char *pdf_dict_get_string(const PdfObj& key, 
			#ifdef SWIG
				size_t *OUTPUT
			#else
				size_t *sizep
	...
	FZ_FUNCTION char *pdf_field_name() const;
	...
	FZ_FUNCTION PdfObj pdf_filter_xobject_instance(const PdfObj& page_res, const FzMatrix& ctm, PdfFilterOptions& filter, const PdfCycleList& cycle_up) const;
	 ...
	 */
	FZ_FUNCTION int pdf_obj_memo(int bit, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *memo
	 ...
	 */
	FZ_FUNCTION const char *pdf_to_string(
			#ifdef SWIG
				size_t *OUTPUT
			#else
				size_t *sizep
	...
	FZ_FUNCTION std::string pdf_field_name2();
...
struct PdfPage
{
	 ...
	 */
	FZ_FUNCTION FzTransition pdf_page_presentation(FzTransition& transition, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *duration
...
struct PdfProcessor
{
	...
	FZ_FUNCTION PdfProcessor(const PdfDocument& doc, const PdfProcessor& chain, const PdfObj& old_res, const PdfObj& new_res, int struct_parents, const FzMatrix& transform, PdfFilterOptions& filter);
	...
	FZ_FUNCTION PdfProcessor(const FzDevice& dev, const FzMatrix& ctm, const char *usage, const PdfGstate& gstate, const FzDefaultColorspaces& default_cs, FzCookie& cookie);
	...
	FZ_FUNCTION void pdf_process_contents(const PdfDocument& doc, const PdfObj& obj, const PdfObj& res, FzCookie& cookie) const;
...
struct PdfProcessor2 : PdfProcessor
{
	...
	FZ_FUNCTION virtual void op_gs_CA(struct ::fz_context *arg_0, float arg_2);
	FZ_FUNCTION virtual void op_gs_SMask(struct ::fz_context *arg_0, struct ::pdf_obj *arg_2, struct ::pdf_obj *arg_3, float *arg_4, int arg_5);
	...
	FZ_FUNCTION virtual void op_Do_form(struct ::fz_context *arg_0, const char *arg_2, struct ::pdf_obj *arg_3, struct ::pdf_obj *arg_4);
