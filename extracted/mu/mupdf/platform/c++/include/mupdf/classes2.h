...
namespace mupdf
{
	...
	*/
	FZ_FUNCTION void fz_bitmap_details(const FzBitmap& bitmap, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *w
	...
	*/
	FZ_FUNCTION size_t fz_buffer_extract(const FzBuffer& buf, 
			#ifdef SWIG
	...
	*/
	FZ_FUNCTION size_t fz_buffer_storage(const FzBuffer& buf, 
			#ifdef SWIG
				unsigned char **OUTPUT
			#else
				unsigned char **datap
	...
	*/
	...
	/**
		...
		given colorspace.
	*/
	FZ_FUNCTION void fz_clamp_color(const FzColorspace& cs, const float *in, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *out
	...
	*/
	FZ_FUNCTION void fz_convert_color(const FzColorspace& ss, const float *sv, const FzColorspace& ds, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *dv
			#endif
			, const FzColorspace& is, const FzColorParams& params);
	...
	*/
	FZ_FUNCTION void fz_convert_separation_colors(const FzColorspace& src_cs, const float *src_color, const FzSeparations& dst_seps, const FzColorspace& dst_cs, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *dst_color
			#endif
			, const FzColorParams& color_params);
	...
	*/
	FZ_FUNCTION FzPixmap fz_decomp_image_from_stream(const FzStream& stm, const FzCompressedImage& image, FzIrect& subarea, int indexed, int l2factor, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *l2extra
	...
	*/
	...
	/**
		...
		*compressed_length will be updated on exit to contain the size
		actually used.
	 */
	FZ_FUNCTION void fz_deflate(unsigned char *dest, 
			#ifdef SWIG
				size_t *OUTPUT
			#else
	...
	*/
	FZ_FUNCTION const char *fz_dom_get_attribute(const FzXml& elt, int i, 
			#ifdef SWIG
				const char **OUTPUT
			#else
				const char **att
	...
	*/
	...
	FZ_FUNCTION void fz_draw_story(const FzStoryS& story, const FzDevice& dev, const FzMatrix& ctm);
	...
	*/
	...
	/**
		...
		the caller.
	*/
	FZ_FUNCTION ::fz_error_cb *fz_error_callback(
			#ifdef SWIG
				void **OUTPUT
			#else
				void **user
	...
	*/
	FZ_FUNCTION void fz_fill_pixmap_with_color(const FzPixmap& pix, const FzColorspace& colorspace, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *color
	...
	*/
	FZ_FUNCTION FzPixmap fz_get_pixmap_from_image(const FzImage& image, FzIrect& subarea, FzMatrix& ctm, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *w
	...
	*/
	FZ_FUNCTION int fz_getopt(int nargc, 
			#ifdef SWIG
				char **OUTPUT
			#else
				char **nargv
	...
	*/
	 ...
	 */
	FZ_FUNCTION int fz_grisu(float f, char *s, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *exp
	...
	*/
	FZ_FUNCTION int fz_has_option(const char *opts, const char *key, 
			#ifdef SWIG
				const char **OUTPUT
			#else
				const char **val
	...
	*/
	...
	/**
		...
		if not encoded).
	*/
	FZ_FUNCTION void fz_image_resolution(const FzImage& image, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *xres
	...
	*/
	FZ_FUNCTION const unsigned char *fz_lookup_base14_font(const char *name, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	...
	*/
	...
	/**
		...
		Returns a pointer to the font file data, or NULL if not present.
	*/
	FZ_FUNCTION const unsigned char *fz_lookup_builtin_font(const char *name, int bold, int italic, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	...
	/**
		...
		Returns a pointer to the font file data, or NULL if not present.
	*/
	FZ_FUNCTION const unsigned char *fz_lookup_cjk_font(int ordering, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	...
	/**
		...
		Returns a pointer to the font file data, or NULL if not present.
	*/
	FZ_FUNCTION const unsigned char *fz_lookup_cjk_font_by_language(const char *lang, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	...
	*/
	 ...
	 */
	FZ_FUNCTION const unsigned char *fz_lookup_noto_emoji_font(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	...
	/**
		...
		Returns a pointer to the font file data, or NULL if not present.
	*/
	FZ_FUNCTION const unsigned char *fz_lookup_noto_font(int script, int lang, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	...
	/**
		...
		configuration in which MuPDF is built.
	*/
	FZ_FUNCTION const unsigned char *fz_lookup_noto_math_font(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	 ...
	 */
	FZ_FUNCTION const unsigned char *fz_lookup_noto_music_font(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	 ...
	 */
	FZ_FUNCTION const unsigned char *fz_lookup_noto_symbol1_font(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	 ...
	 */
	FZ_FUNCTION const unsigned char *fz_lookup_noto_symbol2_font(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	...
	*/
	FZ_FUNCTION unsigned char *fz_new_deflated_data(
			#ifdef SWIG
				size_t *OUTPUT
			#else
				size_t *compressed_length
	...
	/**
		...
		down if it wants to.
	*/
	FZ_FUNCTION unsigned char *fz_new_deflated_data_from_buffer(
			#ifdef SWIG
				size_t *OUTPUT
			#else
				size_t *compressed_length
			#endif
			, const FzBuffer& buffer, ::fz_deflate_level level);
	...
	*/
	FZ_FUNCTION FzDisplayList fz_new_display_list_from_svg(const FzBuffer& buf, const char *base_uri, const FzArchive& zip, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *w
	...
	/**
		Parse an SVG document into a display-list.
	*/
	FZ_FUNCTION FzDisplayList fz_new_display_list_from_svg_xml(const FzXml& xmldoc, const FzXml& xml, const char *base_uri, const FzArchive& zip, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *w
	...
	*/
	FZ_FUNCTION FzImage fz_new_image_from_compressed_buffer(int w, int h, int bpc, const FzColorspace& colorspace, int xres, int yres, int interpolate, int imagemask, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *decode
			...
			, 
				...
				int *colorkey
			#endif
			, const FzCompressedBuffer& buffer, const FzImage& mask);
	...
	*/
	FZ_FUNCTION FzImage fz_new_image_of_size(int w, int h, int bpc, const FzColorspace& colorspace, int xres, int yres, int interpolate, int imagemask, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *decode
			...
			, 
				...
				int *colorkey
			#endif
			, const FzImage& mask, size_t size, ::fz_image_get_pixmap_fn *get_pixmap, ::fz_image_get_size_fn *get_size, ::fz_drop_image_fn *drop);
	...
	*/
	FZ_FUNCTION FzDevice fz_new_svg_device_with_id(const FzOutput& out, float page_width, float page_height, int text_format, int reuse_images, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *id
	...
	*/
	FZ_FUNCTION FzDevice fz_new_test_device(
			#ifdef SWIG
				int *OUTPUT
			#else
				int *is_color
			#endif
			, float threshold, int options, const FzDevice& passthrough);
	...
	*/
	FZ_FUNCTION FzStream fz_open_image_decomp_stream(const FzStream& arg_0, const FzCompressionParams& arg_1, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *l2factor
	...
	*/
	FZ_FUNCTION FzStream fz_open_image_decomp_stream_from_buffer(const FzCompressedBuffer& arg_0, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *l2factor
	...
	*/
	FZ_FUNCTION size_t fz_pack_path(uint8_t *pack, size_t max, const FzPath& path);
	...
	*/
	FZ_FUNCTION FzTransition fz_page_presentation(const FzPage& page, FzTransition& transition, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *duration
	...
	*/
	 ...
	 */
	FZ_FUNCTION const char *fz_parse_page_range(const char *s, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *a
	...
	*/
	...
	FZ_FUNCTION int fz_place_story(const FzStoryS& story, const FzRect& where, FzRect& filled);
	...
	*/
	FZ_FUNCTION FzBuffer fz_read_best(const FzStream& stm, size_t initial, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *truncated
	...
	*/
	...
	FZ_FUNCTION void fz_reset_story(const FzStoryS& story);
	...
	*/
	FZ_FUNCTION FzLocation fz_resolve_link(const FzDocument& doc, const char *uri, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *xp
	...
	*/
	 ...
	 */
	FZ_FUNCTION int fz_search_chapter_page_number(const FzDocument& doc, int chapter, int page, const char *needle, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *hit_mark
			#endif
			, FzQuad& hit_bbox, int hit_max);
	 ...
	 */
	FZ_FUNCTION int fz_search_display_list(const FzDisplayList& list, const char *needle, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *hit_mark
	...
	*/
	FZ_FUNCTION int fz_search_page(const FzPage& page, const char *needle, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *hit_mark
	 ...
	 */
	FZ_FUNCTION int fz_search_page_number(const FzDocument& doc, int number, const char *needle, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *hit_mark
	...
	/**
		...
		without notice.
	*/
	FZ_FUNCTION int fz_search_stext_page(const FzStextPage& text, const char *needle, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *hit_mark
	...
	*/
	FZ_FUNCTION void fz_separation_equivalent(const FzSeparations& seps, int idx, const FzColorspace& dst_cs, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *dst_color
			#endif
			, const FzColorspace& prf, const FzColorParams& color_params);
	...
	*/
	FZ_FUNCTION int fz_store_scavenge(size_t size, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *phase
	...
	/**
		...
		Returns non zero if we managed to free any memory.
	*/
	FZ_FUNCTION int fz_store_scavenge_external(size_t size, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *phase
	...
	FZ_FUNCTION FzXml fz_story_document(const FzStoryS& story);

	/** Class-aware wrapper for `::fz_story_positions()`.  */
	FZ_FUNCTION void fz_story_positions(const FzStoryS& story, ::fz_story_position_callback *cb, void *arg);

	/** Class-aware wrapper for `::fz_story_warnings()`.  */
	FZ_FUNCTION const char *fz_story_warnings(const FzStoryS& story);
	...
	*/
	...
	/**
		...
		"NAN", "INF" or "INFINITY".
	*/
	FZ_FUNCTION float fz_strtof(const char *s, 
			#ifdef SWIG
				char **OUTPUT
			#else
				char **es
	...
	*/
	...
	/**
		...
		the caller.
	*/
	FZ_FUNCTION ::fz_warning_cb *fz_warning_callback(
			#ifdef SWIG
				void **OUTPUT
			#else
				void **user
	...
	*/
	...
	FZ_FUNCTION void fz_write_story(const FzDocumentWriter& writer, const FzStoryS& story, ::fz_write_story_rectfn rectfn, void *rectfn_ref, ::fz_write_story_positionfn positionfn, void *positionfn_ref, ::fz_write_story_pagefn pagefn, void *pagefn_ref);
	...
	*/
	 ...
	 */
	FZ_FUNCTION void pdf_annot_MK_BC(const PdfAnnot& annot, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *n
	 ...
	 */
	FZ_FUNCTION void pdf_annot_MK_BG(const PdfAnnot& annot, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *n
	 ...
	 */
	FZ_FUNCTION void pdf_annot_color(const PdfAnnot& annot, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *n
	 ...
	 */
	FZ_FUNCTION void pdf_annot_default_appearance(const PdfAnnot& annot, 
			#ifdef SWIG
				const char **OUTPUT
			#else
				const char **font
	 ...
	 */
	FZ_FUNCTION void pdf_annot_interior_color(const PdfAnnot& annot, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *n
	 ...
	 */
	FZ_FUNCTION void pdf_annot_line_ending_styles(const PdfAnnot& annot, 
			#ifdef SWIG
	 ...
	 */
	FZ_FUNCTION const char *pdf_array_get_string(const PdfObj& array, int index, 
			#ifdef SWIG
				size_t *OUTPUT
			#else
				size_t *sizep
	...
	FZ_FUNCTION char *pdf_crypt_method(const PdfCrypt& crypt);
	 ...
	 */
	FZ_FUNCTION int pdf_decode_cmap(const PdfCmap& cmap, unsigned char *s, unsigned char *e, 
			#ifdef SWIG
				unsigned int *OUTPUT
			#else
				unsigned int *cpt
	 ...
	 */
	FZ_FUNCTION const char *pdf_dict_get_string(const PdfObj& dict, const PdfObj& key, 
			#ifdef SWIG
				size_t *OUTPUT
			#else
				size_t *sizep
	 ...
	 */
	FZ_FUNCTION int pdf_edit_text_field_value(const PdfAnnot& widget, const char *value, const char *change, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *selStart
	 ...
	 */
	FZ_FUNCTION void pdf_eval_function(const PdfFunction& func, const float *in, int inlen, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *out
	 ...
	 */
	FZ_FUNCTION int pdf_field_event_validate(const PdfDocument& doc, const PdfObj& field, const char *value, 
			#ifdef SWIG
				char **OUTPUT
			#else
				char **newvalue
	...
	FZ_FUNCTION char *pdf_field_name(const PdfObj& field);

	/** Alternative to `pdf_field_name()` that returns a std::string. */
	FZ_FUNCTION std::string pdf_field_name2(PdfObj& field);
	...
	FZ_FUNCTION void pdf_filter_annot_contents(const PdfDocument& doc, const PdfAnnot& annot, PdfFilterOptions& filter);

	/** Class-aware wrapper for `::pdf_filter_page_contents()`.  */
	FZ_FUNCTION void pdf_filter_page_contents(const PdfDocument& doc, const PdfPage& page, PdfFilterOptions& filter);

	/** Class-aware wrapper for `::pdf_filter_xobject_instance()`.  */
	FZ_FUNCTION PdfObj pdf_filter_xobject_instance(const PdfObj& old_xobj, const PdfObj& page_res, const FzMatrix& ctm, PdfFilterOptions& filter, const PdfCycleList& cycle_up);
	 ...
	 */
	FZ_FUNCTION int pdf_js_event_result_validate(const PdfJs& js, 
			#ifdef SWIG
				char **OUTPUT
			#else
				char **newvalue
	 ...
	 */
	FZ_FUNCTION void pdf_js_execute(const PdfJs& js, const char *name, const char *code, 
			#ifdef SWIG
				char **OUTPUT
			#else
				char **result
	 ...
	 */
	FZ_FUNCTION void pdf_load_encoding(
			#ifdef SWIG
				const char **OUTPUT
			#else
				const char **estrings
	 ...
	 */
	FZ_FUNCTION void pdf_load_to_unicode(const PdfDocument& doc, const PdfFontDesc& font, 
			#ifdef SWIG
				const char **OUTPUT
			#else
				const char **strings
	 ...
	 */
	FZ_FUNCTION int pdf_lookup_anchor(const PdfDocument& doc, const char *name, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *xp
	 ...
	 */
	FZ_FUNCTION int pdf_lookup_cmap_full(const PdfCmap& cmap, unsigned int cpt, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *out
	 ...
	 */
	FZ_FUNCTION const unsigned char *pdf_lookup_substitute_font(int mono, int serif, int bold, int italic, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *len
	 ...
	 */
	FZ_FUNCTION void pdf_map_one_to_many(const PdfCmap& cmap, unsigned int one, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *many
	...
	FZ_FUNCTION PdfProcessor pdf_new_filter_processor(const PdfDocument& doc, const PdfProcessor& chain, const PdfObj& old_res, const PdfObj& new_res, int struct_parents, const FzMatrix& transform, PdfFilterOptions& filter);
	...
	FZ_FUNCTION PdfProcessor pdf_new_run_processor(const FzDevice& dev, const FzMatrix& ctm, const char *usage, const PdfGstate& gstate, const FzDefaultColorspaces& default_cs, FzCookie& cookie);
	 ...
	 */
	FZ_FUNCTION int pdf_obj_memo(const PdfObj& obj, int bit, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *memo
	 ...
	 */
	FZ_FUNCTION FzTransition pdf_page_presentation(const PdfPage& page, FzTransition& transition, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *duration
	 ...
	 */
	FZ_FUNCTION void pdf_parse_default_appearance(const char *da, 
			#ifdef SWIG
				const char **OUTPUT
			#else
				const char **font
	 ...
	 */
	FZ_FUNCTION PdfObj pdf_parse_ind_obj(const PdfDocument& doc, const FzStream& f, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *num
	 ...
	 */
	FZ_FUNCTION PdfObj pdf_parse_journal_obj(const PdfDocument& doc, const FzStream& stm, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *onum
	...
	FZ_FUNCTION void pdf_process_contents(const PdfProcessor& proc, const PdfDocument& doc, const PdfObj& obj, const PdfObj& res, FzCookie& cookie);
	...
	This function has out-params. Python/C# wrappers look like:
		`pdf_repair_obj(::pdf_document *doc, ::pdf_lexbuf *buf, ::pdf_obj **encrypt, ::pdf_obj **id, ::pdf_obj **page, ::pdf_obj **root)` => `(int, int64_t stmofsp, int stmlenp, int64_t tmpofs)`
	 */
	FZ_FUNCTION int pdf_repair_obj(const PdfDocument& doc, const PdfLexbuf& buf, 
			#ifdef SWIG
				int64_t *OUTPUT
			#else
				int64_t *stmofsp
	 ...
	 */
	FZ_FUNCTION int pdf_resolve_link(const PdfDocument& doc, const char *uri, 
			#ifdef SWIG
				float *OUTPUT
			#else
				float *xp
	 ...
	 */
	FZ_FUNCTION size_t pdf_signature_contents(const PdfDocument& doc, const PdfObj& signature, 
			#ifdef SWIG
				char **OUTPUT
			#else
				char **contents
	 ...
	 */
	FZ_FUNCTION char *pdf_sprint_obj(char *buf, size_t cap, 
			#ifdef SWIG
				size_t *OUTPUT
			#else
				size_t *len
			#endif
			, const PdfObj& obj, int tight, int ascii);
	 ...
	 */
	FZ_FUNCTION const char *pdf_to_string(const PdfObj& obj, 
			#ifdef SWIG
				size_t *OUTPUT
			#else
				size_t *sizep
	 ...
	 */
	FZ_FUNCTION int pdf_undoredo_state(const PdfDocument& doc, 
			#ifdef SWIG
				int *OUTPUT
			#else
				int *steps
