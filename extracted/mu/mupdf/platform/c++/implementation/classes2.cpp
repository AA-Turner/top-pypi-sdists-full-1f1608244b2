...
FZ_FUNCTION size_t fz_pack_path(uint8_t *pack, size_t max, const FzPath& path)
{
	...
	#endif
	auto ret = mupdf::ll_fz_pack_path(pack, max, path.m_internal);
	return ret;
}
...
FZ_FUNCTION char *pdf_field_name(const PdfObj& field)
{
	...
	#endif
	auto ret = mupdf::ll_pdf_field_name(field.m_internal);
	return ret;
}
...
{
	return ll_pdf_field_name2( field.m_internal);
}
...
FZ_FUNCTION void pdf_filter_annot_contents(const PdfDocument& doc, const PdfAnnot& annot, PdfFilterOptions& filter)
{
	...
	#endif
	mupdf::ll_pdf_filter_annot_contents(doc.m_internal, annot.m_internal,  filter.internal());
}
...
FZ_FUNCTION void pdf_filter_page_contents(const PdfDocument& doc, const PdfPage& page, PdfFilterOptions& filter)
{
	...
	#endif
	mupdf::ll_pdf_filter_page_contents(doc.m_internal, page.m_internal,  filter.internal());
}
...
FZ_FUNCTION PdfObj pdf_filter_xobject_instance(const PdfObj& old_xobj, const PdfObj& page_res, const FzMatrix& ctm, PdfFilterOptions& filter, const PdfCycleList& cycle_up)
{
	...
	#endif
	::pdf_obj* temp = mupdf::ll_pdf_filter_xobject_instance(old_xobj.m_internal, page_res.m_internal, * ctm.internal(),  filter.internal(), cycle_up.m_internal);
	ll_pdf_keep_obj(temp);
	auto ret = PdfObj(temp);
	return ret;
}
...
FZ_FUNCTION PdfProcessor pdf_new_filter_processor(const PdfDocument& doc, const PdfProcessor& chain, const PdfObj& old_res, const PdfObj& new_res, int struct_parents, const FzMatrix& transform, PdfFilterOptions& filter)
{
	...
	#endif
	::pdf_processor* temp = mupdf::ll_pdf_new_filter_processor(doc.m_internal, chain.m_internal, old_res.m_internal, new_res.m_internal, struct_parents, * transform.internal(),  filter.internal());
	auto ret = PdfProcessor(temp);
	return ret;
}
...
FZ_FUNCTION PdfProcessor pdf_new_run_processor(const FzDevice& dev, const FzMatrix& ctm, const char *usage, const PdfGstate& gstate, const FzDefaultColorspaces& default_cs, FzCookie& cookie)
{
	...
	#endif
	::pdf_processor* temp = mupdf::ll_pdf_new_run_processor(dev.m_internal, * ctm.internal(), usage, gstate.m_internal, default_cs.m_internal, &cookie.m_internal);
	auto ret = PdfProcessor(temp);
	return ret;
}
...
FZ_FUNCTION void pdf_process_contents(const PdfProcessor& proc, const PdfDocument& doc, const PdfObj& obj, const PdfObj& res, FzCookie& cookie)
{
	...
	#endif
	mupdf::ll_pdf_process_contents(proc.m_internal, doc.m_internal, obj.m_internal, res.m_internal, &cookie.m_internal);
}
...
/* Class-aware wrapper for `::pdf_repair_obj()`.
	 ...
	 */
FZ_FUNCTION int pdf_repair_obj(const PdfDocument& doc, const PdfLexbuf& buf, int64_t *stmofsp, int *stmlenp, PdfObj& encrypt, PdfObj& id, PdfObj& page, int64_t *tmpofs, PdfObj& root)
{
	#ifndef NDEBUG
	if (s_trace) {
		std::cerr << __FILE__ << ":" << __LINE__ << ":" << __FUNCTION__ << "():"
