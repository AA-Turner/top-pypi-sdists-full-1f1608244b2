...
class ReportingExecutor(DefaultExecutor):
    ...
    def _build_execution_request_result_aspect(
        ...
        if structured_report is not None and ((len(structured_report) + len(report)) > max_length):
            message = f"{exec_request.exec_id} structured report exceeded limit of {max_length} chars and was truncated."
            logger.warning(message)
            structured_report = None
            report = f"WARNING: {message}\n{report}"

        if report is not None and len(report) > max_length:
            message = f"{exec_request.exec_id} report exceeded limit of {max_length} chars and was truncated."
            logger.warning(message)
